...
class Handler(server.ProjectAPIHandler):
    ...
    def flash(self, options):
        if self._is_allhw_ci():
            return self._ci_handler.flash(options)
    ...
    def open_transport(self, options):
        if self._is_allhw_ci():
            return self._ci_handler.open_transport(options)
        if self._is_qemu(options):
    ...
    def close_transport(self):
        if self._is_allhw_ci():
            return self._ci_handler.close_transport()
    ...
    def read_transport(self, n, timeout_sec):
        if self._is_allhw_ci():
            to_return = self._ci_handler.read_transport(n, timeout_sec)
        else:
            if self._transport is None:
                raise server.TransportClosedError()
            to_return = self._transport.read(n, timeout_sec)
        return to_return

    def write_transport(self, data, timeout_sec):
        if self._is_allhw_ci():
            return self._ci_handler.write_transport(data, timeout_sec)
        if self._transport is None:
            raise server.TransportClosedError()
